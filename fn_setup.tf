...
resource "null_resource" "FoggyKitchenSetupATPFnPush2OCIR" {
  depends_on = [local_file.FoggyKitchen_ATP_database_wallet_file, oci_functions_application.FoggyKitchenBackendFnApp, oci_database_autonomous_database.FoggyKitchenATPdatabase]
...
resource "null_resource" "FoggyKitchenBackendFnPush2OCIR" {
  depends_on = [local_file.FoggyKitchen_ATP_database_wallet_file, oci_streaming_stream.FoggyKitchenStream, oci_streaming_stream_pool.FoggyKitchenStreamPool, oci_functions_application.FoggyKitchenBackendFnApp, oci_database_autonomous_database.FoggyKitchenATPdatabase, null_resource.FoggyKitchenSetupATPFnPush2OCIR]
...
resource "null_resource" "FoggyKitchenFrontendFnPush2OCIR" {
  ...
  #provisioner "local-exec" {
  #  command = "image=$(docker images | grep frontendfn | awk -F ' ' '{print $3}') ; docker rmi -f $image &> /dev/null"
  ...
  provisioner "local-exec" {
    command = "image=$(docker images | grep frontendfn | awk -F ' ' '{print $3}') ; docker tag $image ${var.ocir_docker_repository}/${var.ocir_namespace}/${var.ocir_repo_name}/frontendfn:0.0.1"
    working_dir = "functions/FrontendFn"
  }
